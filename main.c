...
#ifdef __linux__    //현재 실행 환경이 리눅스인지 확인, 리눅스 환경(onlinGDB)이라면 unistd.h를 사용
    #include <unistd.h> //출력 버퍼 비우기, sleep으로 화면 대기를 위해 사용 
    #define sleep(s) sleep(s)   //리눅스면 sleep 그냥 그대로 내비둠
   
#elif _WIN32  //리눅스 아니고 윈도우라면, 윈도우 환경에서는 windows.h를 사용
    #define _CRT_SECURE_NO_WARNINGS //scanf, fscanf 등 사용시 경고문구 X, 코드 정상적으로 진행
    #include <windows.h>    //출력 버퍼 비우기, Sleep으로 화면 대기를 위해 사용 
    #define sleep(s) Sleep((s)*1000)    //윈도우면 <unistd.h> sleep을 <windows.h> Sleep로 바꿔줌 / Sleep은 ms단위로 대기하기 때문에 1000을 곱해줌
#endif  //ifdef 코드 끝
...
#include "ingredient.h" //게임 변수들과 재료 도트 배열, 재료 출력과 관련된 함수가 모여있는 헤더파일, 코드 처음 작성할때 재료 출력 부분이 너무 길어져서 따로 헤더파일을 만들었음

// 각종 변수, 재료 출력과 선택에 관한 함수는 "ingredient.h"에 있음 

//"main.c" 함수 목록
void intro();   //게임 시작화면을 출력하기 위한 함수
...
void cpyIngredient(char ingredientName[MAX_IN][20], char list[NEED_MAX]);  //(손님의 랜덤 주문 또는 플레이어)가 선택한 재료를 (손님 또는 플레이어) 구조체의 문자열 멤버에 복사하는 함수 
...
void ShowScore(int correct, int plusScore, int plusGold,int *currentGold);   //맞춘 정도 수치에 따라 계산한 손님 만족도, 획득 점수, 획득 금액을 출력
void ShowEnding(int finalGold, int unlockCount);  //엔딩 출력

int main() {    //main

    int customerCount = ((currentStage * 2) + DEFAULT_CUSTOMER);    //스테이지 별 손님 수 = 현재 스테이지*2 * 기본 손님 수 
	int gold = INITIAL_GOLD;    //뵤유 금액 = 초기 금액
	int unlockTotal = 0;   //해금된 재료 수 확인하기 위한 변수(엔딩 조건 검사를 위해서)

    intro();    //인트로 출력    

    while (currentStage <= 3) { // 최대 스테이지인 3스테이지까지만 반복

        customerCount = ((currentStage * 2) + DEFAULT_CUSTOMER);    //스테이지 별 손님 수 = 현재 스테이지*2 * 기본 손님 수 
        
        printStage(customerCount,&gold,&unlockTotal);   //현재 스테이지(2스테이지 이후부터는 해금단계도) 출력
        cooking(customerCount, &gold);  //요리 과정 진행, 출력
        currentStage++; //다음 스테이지로

    }
    printStage(customerCount, &gold,&unlockTotal);   //엔딩 전 두 번째 엔딩 조건 충족을 위한 재료 해금을 위해 한번 더 스테이지(해금단계) 출력
...
void intro() {   //게임 시작시 한번 나오는 튜토리얼을 위한 함수
    ...
    unsigned int argb = 0, a = 0, r = 0, g = 0, b = 0; //각 재료의 a(투명도 : 보임 or 안보임),rgb값을 저장할 변수
    ...
    fp = fopen("INTRO.txt", "rt");   //INTRO 텍스트파일 열기

    for (int i = 0; i < 13; i++) { //각 행 반복(위에서 아래로)
        for (int k = 0; k < 50; k++) { //각 열 반복(왼쪽에서 오른쪽으로)
            
            //텍스트 파일에 한 도트당 8자리 16진수 정수 0x??/??/??/??로 저장됨 
            //16진수임을 나타내는 0x 바로 왼쪽 ??이 a(00 or ff : 투명 or 보임), 그다음 ??은 b(0~ff), 다음 ??은 g(0~ff), 마지막 ??은 r(0~ff)값. 16진수 2자리(0~ff)는 10진수로(0~255)임.

            fscanf(fp, "%x", &argb);    //16진수 형식으로 읽어오기 위해서 %d 가 아닌 %x / 16진수로 저장한 것을 10진수로 표현하면 숫자는 바뀌지만 수치 자체는 동일하기 때문에 10진수 연산 가능 ex) 0xf == 15
            a = argb / (256 * 256 * 256); //16진수 2자리(00~ff)는 10진수로 3자리(0~255)로 표현되기 때문에 256으로 나눗셈, 나머지 연산해서 16진수의 각 2자리마다 수치를 뽑아낼 수 있음.
...
// 현재 스테이지를 출력하고 2스테이지 부터는 재료 해금 단계도 출력하는 함수
void printStage(int maxCustomer,int *currentGold,int *unlockCount) {  //최대 손님수, 현재 보유 금액 변수의 주소(해금단계에서 금액 소모가 변수에 반영되도록), 현재 해금한 재료 개수 변수의 주소(해금단계에서 해금했을 때 +1가 변수에 반영 되도록)

    clearDisplay();  //화면 전체를 지움 "ingredient.h"에 있는 함수
    printf("\033[?25l");    //깜빡이는 커서를 보이지 않게함
    ...
    if ( (currentStage > 1) && (currentStage <= 4) ){  // (2스테이지 부터 3스테이지 종료 후)까지 재료 해금단계 출력 
    ...
    printf("\033[?25h"); //깜빡이는 커서를 다시 보이게 함
...
void cpyIngredient(char ingredientName[MAX_IN][20], char list[NEED_MAX]) {   //함수의 전달인자로 배열을 받을 땐 그 주소값이 들어 오기 때문에 변경 값이 배열에도 적용 됨
    ...
    strcpy(ingredientName[BREAD], currentBread[list[BREAD]]);                //(손님 랜덤 또는 플레이어 선택)의 list[]에는 재료 번호(채소의 경우 0~5)가 저장, ingredient[]문자열에는 번호에 해당하는 재료 이름("양상추"~"피클")이 저장되어 있음 
    ...
    strcpy(ingredientName[VEGETABLE1], currentVegetable[list[VEGETABLE1]]);  //BREAD(0)~DRINK(6) 인덱스로 list[]의 각 재료 유형ex)VEGETABLE2 에 접근해 
    strcpy(ingredientName[VEGETABLE2], currentVegetable[list[VEGETABLE2]]);  //저장되어 있는 재료의 번호 ex)list[VEGETABLE2] = 0 이라면
    strcpy(ingredientName[SAUCE1], currentSauce[list[SAUCE1]]);              //current재료([0]="양상추")를 (손님 랜덤 또는 플레이어 선택)의 ingredient([0]="양상추")로 복사
...
void displayCustomer() {
    ...
    if ((currentVegetable[Need.list[VEGETABLE1]][0] != '\0') && (Need.list[VEGETABLE1] == Need.list[VEGETABLE2])) {// 만약 채소가 해금 되어 있고 동일한 2가지 채소(2스테이지부터 나오는)라면 
        printf("%s 2단/ ", currentVegetable[Need.list[VEGETABLE1]]); //"채소이름 2단, " 출력 
        Need.total += 2;    //주문 재료 수 +2
    }
    else {//동일한 채소가 아니면 각 랜덤 채소를 출력
    ...
    if ((currentSauce[Need.list[SAUCE1]][0] != '\0') && (Need.list[SAUCE1] == Need.list[SAUCE2])) {// 만약 소스가 해금 되어 있고 동일한 2가지 소스(3스테이지부터 나오는)라면 
    ...
    cpyIngredient(Need.ingredient, Need.list);   //손님 재료 문자열과 번호 문자열 주소를 함수에 전달해 랜덤으로 정해진대로 문자열 변경
                                                //랜덤 채소와 소스가 모두 해금되어 있지 않아서 기본 재료로 변경하는 경우가 있기 때문에 주문 텍스트 다 출력 후 값을 저장
    
    for (int i = 4; i >= 1; i--) {    //4 3 2 1 출력하는 반복문
        printf("\033[1m\033[6;20f%d", i);    // 반복에 따라 4->3->2->1 출력
        fflush(stdout); //printf 내용이 버퍼에 남아 화면에 안보이는 경우가 있기 때문에 출력버퍼를 비워줌
        sleep(1);   //4 3 2 1 화면에 표시하기 위한 1초 대기
    }
    ...
    for (int i = 0; i < NEED_MAX; i++)   //손님 주문 재료 하나하나 플레이어 선택 재료 전체와 비교, 주문 최대치 만큼 반복
        for (int k = 0; k < NEED_MAX; k++)
            ...
            if ((Choose.ingredient[k][0] != '\0') && (!strcmp(Need.ingredient[i], Choose.ingredient[k]))) {
                ...
                strcpy(Need.ingredient[i], "\0");// 점수 출력 시 플레이어가 틀린 재료만 확인시켜주기 위해서 맞춘 재료이름 문자열 삭제
                strcpy(Choose.ingredient[k], "\0");//중복 카운트를 막기 위해서 문자열 삭제 
...
void ShowScore(int correct, int plusScore, int plusGold, int *currentGold) {// 맞춘 정도 수치, 획득 점수, 획득 금액, 현재 보유금액 변수의 주소(획득 금액을 더한 값을 변수에 적용하기 위함)
    ...
    printf("\033[?25l");    //깜빡이는 커서를 보이지 않게함
    printf("\033[2;1f\033[40m%d단계 >> 점수 : %d/%d  돈 : %d  \033[0m", currentStage, score,CLEAR_SCORE, *currentGold); //현재 스테이지, 점수, 보유 금액 출력
    ...
    printf("\033[7;15f\033[1m         %s\033[0m", customerFeeling[correct - 1]);   //손님 만족도 (맞춘정도 = 1~5, 인덱스는 0부터)  
    fflush(stdout); //출력 버퍼 비워줌
    sleep(1);   //화면에 일정 시간 간격을 두고 표시하기 위한 대기
    
    printf("\033[9;15f점수 : +%d", plusScore);   //획득 점수(외부에서 계산해서 전달)
    score += plusScore;     //현재 점수에 획득 점수 추가
    printf("\033[2;1f\033[40m%d단계 >> 점수 : %d/%d  돈 : %d  \033[0m", currentStage, score,CLEAR_SCORE, *currentGold); //갱신된 점수 출력
    fflush(stdout); //출력 버퍼 비워줌
    sleep(1);   //화면에 일정 시간 간격을 두고 표시하기 위한 대기
    
    printf("\033[10;15f돈 : +%d", plusGold);     //획득 금액(외부에서 계산해서 전달)
    *currentGold += plusGold;   //보유 금액에 획득 금액 추가
    printf("\033[2;1f\033[40m%d단계 >> 점수 : %d/%d  돈 : %d  \033[0m", currentStage, score,CLEAR_SCORE, *currentGold); //갱신된 보유 금액 출력
    fflush(stdout); //출력 버퍼 비워줌
    sleep(1);   //화면에 일정 시간 간격을 두고 표시하기 위한 대기
}

// 요리 과정 출력, 플레이어 재료 선택하는 함수
void cooking(int maxCustomer, int *currentGold) {// 최대 손님수, 현재 보유 금액 포인터(변경값 적용을 위해서) 전달

    int correct = 0;    //맞춘 정도 수치(만족도 5단계) 초기화
    int increaseScore = 0; //요리 한 번마다 획득하는 점수는 만족도5단계 * increaseScore 
	int increaseGold = 0; //요리 한번마다 획득하게 되는 금액 = {요리기본금액 * (손님이 선택한 고기(0~2) +1)}  +  (맞춘정도 * 추가금액)

    int durationOfPlayer = TIME_LIMIT * currentStage;  //요리 제한 시간(남은시간) = 기본제한시간 * 현재 스테이지
    int customerCount = maxCustomer;    //나오는 손님 수(남은 손님) = (기본손님 * 현재 스테이지)
    ...
    do {
		...
		//점수 출력 - 맞춘 정도 수치, 획득 점수(맞춘 정도 * 기본획득점수), 획득 금액, 보유 금액 포인터
        ShowScore(correct , increaseScore , increaseGold , currentGold);

        durationOfPlayer -= (time(NULL) - duration);    //남은시간 -= 걸린 시간(현재 시간 - 소요 시간 측정 당시 시간)
        (durationOfPlayer < 0) ? durationOfPlayer = 0 : durationOfPlayer;   //남은시간이 0이하면 0으로 (남은시간 -20 이런 식이면 이상하기 때문)
        customerCount--; //남은 손님 - 1

        printf("\033[11;15f남은 손님      : %d명",customerCount);    //남은 손님 출력
        printf("\033[12;15f남은 시간      : %d초", durationOfPlayer);  //남은 시간 출력
        fflush(stdout); //printf 내용이 버퍼에 남아 화면에 안보이는 경우가 있기 때문에 출력버퍼를 비워줌
...
void ShowEnding(int finalGold,int unlockCount) {
    ...
    printf("\033[2;1f\033[40m%d단계 >> 점수 : %d/%d  돈 : %d  \033[0m", currentStage, score,CLEAR_SCORE, finalGold);   //현재 스테이지, 현재 점수, 보유 금액 출력
